...
{
    ?>
<style>@media screen and (min-width: 1000px){body {background: var(--body-bg-color);} :root{--body-bg-color:url(<?php 
    echo MJ('zibll_Add_ons_body_backgroundtu333');
    ?>) top fixed;}}</style>
<?php 
}
//网站背景mp1
...
{
    ?>
<style>@media screen and (max-width: 1000px){body {background: var(--body-bg-color);} :root{--body-bg-color:url(<?php 
    echo MJ('zibll_Add_ons_body_backgroundtu666');
    ?>) top fixed;}}</style>
<?php 
}
//下雪特效35
...
{
    ?>
<script>var OriginTitile = document.title,titleTime;document.addEventListener("visibilitychange",function() {if (document.hidden) {document.title = "<?php 
    echo MJ('dingbubiaoti_mj1');
    ?>"; clearTimeout(titleTime) } else {document.title = "<?php 
    echo MJ('dingbubiaoti_mj2');
    ?>" ; titleTime = setTimeout(function(){ document.title = OriginTitile}, 2000)}});</script>
<?php 
}
//顶部你别走呀
...
{
    ?>
<script>var keyWords = "—<?php 
    echo MJ('dongtaibiaoti1');
    ?>—";function titleChange() {var keyList = keyWords.split("");var firstChar = keyList.shift();keyList.push(firstChar);keyWords = keyList.join("");document.title = keyWords;}setInterval(titleChange, 500);</script>
<?php 
}
//手机侧边栏1
...
{
    ?>
<style>@media (max-width: 767px){.mobile-navbar.show,.mobile-navbar.left{background-size: cover;background-repeat: no-repeat;background-position: center center;cursor: pointer;background-image:linear-gradient(rgba(255, 255,255,0),rgba(255,255,255, 0.3)),url("<?php 
    echo MJ('shoujicebianlan333');
    ?>");}.mobile-nav-widget .box-body {background: var(--muted-border-color) !important;}}</style>
<?php 
}
//手机侧边栏2
...
<a href="http://wpa.qq.com/msgrd?v=3&uin=<?php 
        echo $contact_help_qq;
        ?>&site=qq&menu=yes"target="_blank" class="contact-help-qq" style="font-weight:700;">联系站长<svg class="icon" aria-hidden="true"><use xlink:href="#icon-QQ"></use></svg></a>
<?php 
    }
}
// 随便看看
...
//时间
function zibll_Add_ons_timeauthor()
{
    ?>
...
<div class="footwaveline">
    ...
    <div id="shi" style="position: fixed;bottom: 0;margin-left: 200px;z-index: 99;">
        <h4 class="my-face" style="font-size:13px;margin:0 5px 2px 5px;color:#797979;margin-bottom: 10px;"><?php 
    echo MJ('bottomnav0');
    ?></h4>
    </div>
...
<div class="showpc"><style>#wpon_svip_gif {display: none;width: 200px;height: 61px;border-radius: 0 0 13px 13px;z-index: 1002;right: 30px;top: 100px;position: fixed;}.showpc{display:none;}@media (min-width:960px){.showpc{display:inline;}}</style><div id="wpon_svip_gif" class="wpon_svip_gif" style="display: block;"><div class="kubao" style="display: none;"></div><p><a class="link animated" href="<?php 
    echo MJ('timeauthor3');
    ?>" target="_blank" style="border: none; box-shadow: none;"><br><img src="./wp-content/plugins/zibll_Add_ons/img/advertisement.gif" style="margin-top: 500px;" height="200" width="600"><br></a></p></div></div>
<?php 
}
//蒲公英
...
<div class="showpc"><style>#wpon_svip_gif {display: none;width: 100px;height: 125px;border-radius: 0 0 13px 13px;z-index: 1002;right: 30px;top: 110px;position: fixed;}.showpc{display:none;}@media (min-width:960px){.showpc{display:inline;}}</style><div id="wpon_svip_gif" class="wpon_svip_gif" style="display: block;"><div class="kubao" style="display: none;"></div><p><a class="link animated" href="<?php 
    echo MJ('advertisement1');
    ?>" target="_blank" style="border: none; box-shadow: none;"><br><img src="./wp-content/plugins/zibll_Add_ons/img/advertisement.gif" style="margin-top: 500px;" height="100" width="400"><br></a></p></div></div>
<?php 
}
//底线
